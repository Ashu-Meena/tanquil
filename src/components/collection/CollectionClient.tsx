    ...
    <div className="space-y-10">
      {hasActiveFilters && (
        <button
          ...
          className="text-xs uppercase tracking-widest text-sale hover:underline flex items-center gap-1"
        >
      ...
      {availableCategories.length > 0 && (
        <div>
          <h3 className="font-serif text-xl mb-4 border-b border-border-light pb-2">Category</h3>
              ...
              <AnimatePresence>
                {isSortOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -8 }}
                    className="absolute top-full right-0 mt-2 bg-white border border-border-light shadow-lg z-30 min-w-[200px]"
                  >
                    {SORT_OPTIONS.map(opt => (
                      <button
                        key={opt.value}
                        onClick={() => { setSortBy(opt.value); setIsSortOpen(false); }}
                        className={`w-full text-left px-4 py-3 text-xs hover:bg-ivory transition-colors normal-case tracking-normal ${sortBy === opt.value ? 'text-gold font-medium' : 'text-rich-black'}`}
                      >
                        {opt.label}
                      </button>
                    ))}
          ...
          <div className="flex-1">
            {sorted.length === 0 ? (
              <div className="text-center py-20">
                ...
                <button onClick={clearFilters} className="text-gold underline underline-offset-4 text-sm">
                  Clear Filters
                </button>
              </div>
            ) : (
            ...
            {/* Load More */}
            {hasMore && (
              <div className="mt-16 border-t border-border-light pt-10 flex justify-center">
                <button
                  onClick={() => setVisibleCount(c => c + PAGE_SIZE)}
                  className="w-full md:w-auto border border-rich-black px-10 py-4 uppercase tracking-widest text-sm font-medium hover:bg-rich-black hover:text-white transition-colors duration-300"
                >
      ...
      <AnimatePresence>
        {isMobileFilterOpen && (
          <>
            ...
            >
              <div className="flex justify-between items-center p-4 sm:p-6 border-b border-border-light sticky top-0 bg-white">
              ...
              <div className="p-4 sm:p-6 border-t border-border-light mt-auto sticky bottom-0 bg-white">
                <button
                  onClick={() => setIsMobileFilterOpen(false)}
