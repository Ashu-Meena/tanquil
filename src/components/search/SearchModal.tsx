        ...
        <>
          ...
          >
            ...
            <div className="container mx-auto px-6 lg:px-12 py-8 max-w-5xl">
              {/* Header & Input */}
              <div className="flex items-center gap-4 border-b-2 border-rich-black pb-4 mb-12">
                <Search className="w-8 h-8 text-rich-black flex-shrink-0" />
                <input 
                  type="text" 
                  placeholder="Search for dresses, tops, collections..." 
                  value={query}
                  onChange={e => setQuery(e.target.value)}
                  className="flex-1 text-2xl md:text-4xl font-serif bg-transparent outline-none placeholder:text-border-light transition-all"
                  autoFocus
                />
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
                ...
                <div className="md:col-span-4 space-y-10">
                  <div>
                    <h3 className="text-sm font-bold uppercase tracking-widest text-neutral-500 mb-6">
                      ...
                      {trendingSearches.map((term, i) => (
                        <li key={i}>
                          <button
                            ...
                            className="text-lg hover:text-gold transition-colors flex items-center gap-2 group text-left"
                          >
                            {term}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
                      {results.map((product) => (
                        <Link key={product.id} href={`/products/${product.slug}`} onClick={closeSearch} className="group cursor-pointer">
                          <div className="aspect-[3/4] relative bg-ivory mb-4 overflow-hidden">
                            <Image 
                              src={product.image} 
                              alt={product.name} 
                              fill 
                          ...
                          <h4 className="font-medium mb-1 group-hover:text-gold transition-colors">{product.name}</h4>
                          <p className="text-neutral-500">₹{product.price.toLocaleString('en-IN')}</p>
                        </Link>
                      ))}
                    </div>
                  )}
