...
export default function EditProductPage() {
    ...
    if (product) {
      ...
      if (product.product_variants && product.product_variants.length > 0) {
        setVariants(product.product_variants.map((v: any) => {
          const colorKey = v.color_name || "Default";
          return {
            id: v.id,
            ...
            sku: v.sku || "",
            image_urls: colorImages[colorKey] || []
          };
        }));
      } else {
        setVariants([{ id: crypto.randomUUID(), color_name: "Default", color_hex: "#000000", size: "OS", stock_quantity: 0, sku: "", image_urls: [] }]);
      }
    }
    setFetching(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Update Images Grouped by Color (Delete all and re-insert)
