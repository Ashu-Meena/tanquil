...
const allCollectionProducts = Array.from({ length: 24 }).map((_, i) => ({
  id: `col-prod-${i}`,
  ...
  isNew: i % 3 === 0,
  isSale: i % 5 === 0,
  category: i % 3 === 0 ? "Dresses" : i % 3 === 1 ? "Co-ord Sets" : "Tops",
...
export default function CollectionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Apply filters
  let filtered = allCollectionProducts;
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    if (sortBy === "newest") return parseInt(b.id.split('-')[2]) - parseInt(a.id.split('-')[2]);
    ...
    <div className="space-y-10">
      ...
      <div>
        ...
        <ul className="space-y-3 text-[#666666]">
          {["Dresses", "Co-ord Sets", "Tops"].map(cat => (
            <li key={cat}>
